...
class WPSight_Template {
    ...
    public static function get_orderby($args = array()) {
        ...
        $defaults = array(
            ...
            'order' => true,
            'labels' => array(
                'orderby' => _x('Ordenar', 'listings panel actions', 'wpcasa'),
                'date' => _x('Data', 'listings panel actions', 'wpcasa'),
                'orderby_sep' => _x('or', 'listings panel actions', 'wpcasa'),
                'price' => _x('Preço', 'listings panel actions', 'wpcasa'),
                'order' => _x('Ordenar', 'listings panel actions', 'wpcasa'),
                'desc' => _x('Decrescente', 'listings panel actions', 'wpcasa'),
                'order_sep' => _x('or', 'listings panel actions', 'wpcasa'),
                'asc' => _x('Crescente', 'listings panel actions', 'wpcasa'),
                'title' => _x('Titulo', 'listings panel actions', 'wpcasa')
            )
        );
    ...
    public static function get_archive_title() {
        ...
        if (isset($wpsight_query->found_posts)) {
            $title = '<span class="listings-panel-found">' . $wpsight_query->found_posts . '</span>' . ' ' . _nx('Listing', 'Imóveis', intval($wpsight_query->found_posts), 'archive title', 'wpcasa');
        } elseif (isset($wp_query->found_posts)) {
            $title = '<span class="listings-panel-found">' . $wp_query->found_posts . '</span>' . ' ' . _nx('Listing', 'Imóveis', intval($wp_query->found_posts), 'archive title', 'wpcasa');
        } else {
            $title = __('Imóveis', 'wpcasa');
        }
    ...
    public static function get_pagination($max_num_pages = '', $args = array()) {
        ...
        $defaults = array(
            ...
            'mid_size' => 4,
            'prev_next' => true,
            'prev_text' => $arr_prev . __('Anterior', 'wpcasa'),
            'next_text' => __('Proximo', 'wpcasa') . $arr_next,
            'type' => 'list',
            'add_args' => false,
            'add_fragment' => '',
            'before_page_number' => '',
